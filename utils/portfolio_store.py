...
SESSION_KEY = "portfolio_positions_v1"
...
def _clean(df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows=[]
    for (account,ticker,category), group in x.groupby(["Account","Ticker","Category"], dropna=False):
        total_shares=float(group["Shares"].sum())
        avg_cost=float((group["Shares"]*group["Avg Cost"]).sum()/total_shares) if total_shares else 0.0
        rows.append({"Account":account,"Ticker":ticker,"Shares":total_shares,"Avg Cost":avg_cost,"Category":category})
...
def load(force=False):
    ...
    if c:
        try:
            r=requests.get(f"{c['url']}/rest/v1/{c['table']}",headers=_headers(c),params={"profile_id":f"eq.{c['profile']}","select":"account,ticker,shares,avg_cost,category","order":"account.asc,ticker.asc"},timeout=12)
            r.raise_for_status(); rows=r.json()
            if rows:
                out=_clean(pd.DataFrame([{"Account":z.get("account"),"Ticker":z.get("ticker"),"Shares":z.get("shares"),"Avg Cost":z.get("avg_cost"),"Category":z.get("category")} for z in rows]))
...
def save(df):
    ...
    if c:
        try:
            ...
            if not x.empty:
                rows=[{"profile_id":c["profile"],"account":r["Account"],"ticker":r["Ticker"],"shares":float(r["Shares"]),"avg_cost":float(r["Avg Cost"]),"category":r["Category"],"updated_at":datetime.now(timezone.utc).isoformat()} for _,r in x.iterrows()]
